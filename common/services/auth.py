...
class AuthService:
    def __init__(self, config):
        ...
        self.EMAIL_TRANSMITTER_QUEUE_NAME = (
            config.QUEUE_NAME_PREFIX + config.EMAIL_SERVICE_PROCESSOR_QUEUE_NAME
        )
    ...
    def signup(self, email, first_name, last_name):
        ...
        organization = Organization(name=f"{first_name}'s Organization")
        ...
        person_organization_role = (
            self.person_organization_role_service.save_person_organization_role(
                person_organization_role
            )
        )
    ...
    def prepare_password_reset_url(self, login_method: LoginMethod, email: str):
        ...
        password_reset_url = (
            self.config.VUE_APP_URI + "/set-password/" + token + "/" + uid
        )
    ...
    def login_user_by_email_password(self, email: str, password: str):
        ...
        login_method = self.login_method_service.get_login_method_by_email_id(
            email_obj.entity_id
        )
    ...
    def parse_access_token(self, access_token: str) -> dict:
        try:
            decoded_token = jwt.decode(
                access_token, self.config.AUTH_JWT_SECRET, algorithms=["HS256"]
            )
    ...
    def send_password_reset_email(self, email: str, login_method: LoginMethod):
        if password_reset_url := self.prepare_password_reset_url(login_method, email):
            message = {
                ...
                "data": {"verify_link": password_reset_url},
                "to_emails": [email],
            }
            self.message_sender.send_message(self.EMAIL_TRANSMITTER_QUEUE_NAME, message)
    ...
    def reset_user_password(self, token: str, uidb64: str, password: str):
        ...
        new_login_method = LoginMethod(
            method_type=LoginMethodType.EMAIL_PASSWORD, raw_password=password
        )
        ...
        login_method = self.login_method_service.update_password(
            login_method, new_login_method.password
        )
